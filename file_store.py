...
class FileStore(object):
    ...
    def get_bucket_folder(self, bucket_name):
        return fs.path.join(self.root, bucket_name)
    ...
    def create_bucket(self, bucket_name):
        if bucket_name not in [bucket.name for bucket in self.buckets]:
            ...
            target = fs.path.join(self.root, bucket_name)
            if not self.fs.isdir(target):
                self.fs.makedir(target,recursive=True)
    ...
    def delete_bucket(self, bucket_name):
        ...
        self.fs.removedir(fs.path.join(self.root, bucket_name))
    ...
    def get_item(self, bucket_name, item_name):
        key_name = fs.path.join(bucket_name, item_name)
        dirname = fs.path.join(self.root, key_name)
        filename = fs.path.join(dirname, CONTENT_FILE)
        ...
        item.io = self.fs.open(filename, 'rb')

        return item

    def copy_item(self, src_bucket_name, src_name, bucket_name, name, handler):
        src_key_name = fs.path.join(src_bucket_name, src_name)
        src_dirname = fs.path.join(self.root, src_key_name)
        src_filename = fs.path.join(src_dirname, CONTENT_FILE)
        src_meta = self.redis.hgetall(src_key_name)

        bucket = self.get_bucket(bucket_name)
        key_name = fs.path.join(bucket.name, name)
        dirname = fs.path.join(self.root, key_name)
        filename = fs.path.join(dirname, CONTENT_FILE)
        ...
        self.fs.copy(src_filename, filename, overwrite=True)

        return S3Item(key_name, **src_meta)

    def store_data(self, bucket, item_name, headers, data):
        key_name = fs.path.join(bucket.name, item_name)
        dirname = fs.path.join(self.root, key_name)
        filename = fs.path.join(dirname, CONTENT_FILE)
        ...
        s3_item.io = self.fs.open(filename, 'rb')
        return s3_item
